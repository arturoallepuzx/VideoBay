...
final readonly class ListUserWishlistResponse
{
    ...
    /**
     * @param  array{items: list<WishlistItem>, total: int, page: int, totalPages: int}  $result
     * @param  array<string, MovieListItemView>  $views
     */
    public static function create(array $result, array $views): self
    {
        $items = array_map(
            fn (WishlistItem $item): array => self::itemToArray($item, $views),
            $result['items'],
        );
     ...
     * @return array<string, mixed>
     */
    private static function itemToArray(WishlistItem $item, array $views): array
    {
