...
class EloquentWatchLaterItemRepository implements WatchLaterItemRepositoryInterface
{
    private const TABLE = 'watch_later_items';
    ...
    public function listByUser(Uuid $userId, int $page, int $perPage): array
    {
        ...
        $items = [];
        foreach ($rows as $row) {
            $items[] = WatchLaterItem::fromPersistence(
                $userId->value(),
